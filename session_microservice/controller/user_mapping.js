...
export const userMapping = async (req, res, next) => {
  ...
  fetch(
    userMappingServiceURL +
      "/group" +
      "?" +
      new URLSearchParams({ topicId: req.query.topicId })
  ).then((response) => {
    response.json().then((data) => {
      data.member.forEach(async (user) => {
        ...
        Session.findOne({ member: userId })
          .populate("member connection")
          .exec((err, document) => {
            if (err) {
              console.log(error);
...
export const updateConnection = async (req, res, next) => {
  ...
  if (!userId) {
    return createNewUser(user)
      .then(async (user) => createNewSession(userId, connectionId))
      .then((session) => {
